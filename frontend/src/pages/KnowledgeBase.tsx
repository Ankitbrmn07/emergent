...
import { BookOpen, Upload, Search, FileText, CheckCircle2, Trash2, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400">Upload documents (PDF, TXT, DOCX, CSV, MD, JSON) to index vector chunks for agent reasoning context</p>
        </div>
        ...
        <form onSubmit={handleCreateKb} className="flex items-center space-x-2">
          <input
            ...
            className="px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 shadow-sm"
          />
          <button
            type="submit"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <h2 className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Knowledge Stores</h2>
            ...
            {knowledgeBases.map(kb => (
              ...
              >
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-sm text-slate-900 dark:text-white">{kb.name}</h3>
                  <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[10px] text-slate-700 dark:text-slate-400 font-mono">
                    {kb.total_documents} Docs
                  </span>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-1">{kb.description || 'Vector knowledge repository'}</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          {selectedKb && (
            <>
              ...
              <div className="glass-panel rounded-2xl p-5 border border-slate-200 dark:border-slate-800 space-y-4">
                <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 pb-3">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">PDF, TXT, DOCX, CSV, Markdown, JSON supported</p>
                  </div>
                  ...
                  <label className="px-4 py-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-medium text-xs shadow-lg shadow-indigo-600/20 cursor-pointer flex items-center space-x-2 transition-all">
                    ...
                    <span className="text-white">{isUploading ? 'Indexing...' : 'Upload Document'}</span>
                ...
                <div className="space-y-2 max-h-56 overflow-y-auto">
                  {documents.length === 0 ? (
                    ...
                    documents.map(doc => (
                      <div key={doc.id} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                        ...
                        <div className="flex items-center space-x-3">
                          <span className="px-2 py-0.5 rounded-full text-[10px] bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 flex items-center space-x-1">
                            <CheckCircle2 className="w-3 h-3" />
                            <span>Indexed</span>
                          </span>
                          <button onClick={() => handleDeleteDoc(doc.id)} className="text-slate-400 hover:text-rose-600 dark:hover:text-rose-400">
              ...
              <div className="glass-panel rounded-2xl p-5 border border-slate-200 dark:border-slate-800 space-y-4">
                <div className="border-b border-slate-200 dark:border-slate-800/80 pb-3">
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">Test semantic cosine vector query retrieval against indexed document chunks</p>
                </div>
                ...
                <form onSubmit={handleVectorSearch} className="flex items-center space-x-3">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 font-mono shadow-sm"
                  />
                  <button
                    type="submit"
                  ...
                  >
                    ...
                    <span className="text-white">Search Vectors</span>
                  </button>
                </form>
                ...
                {searchResults.length > 0 && (
                  <div className="space-y-3 font-mono text-xs pt-2">
                    {searchResults.map((res, idx) => (
                      <div key={idx} className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-indigo-300 dark:border-indigo-500/30 space-y-1.5">
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-purple-700 dark:text-purple-300 font-bold">Doc: {res.document_filename}</span>
                        ...
                        <p className="text-[11px] text-slate-800 dark:text-slate-300 font-sans leading-relaxed">{res.content}</p>
                      </div>
                    ))}
                  </div>
                )}
