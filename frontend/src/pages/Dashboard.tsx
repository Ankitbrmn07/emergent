...
export const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner Header */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-r from-purple-900/30 via-slate-900 to-indigo-900/30 border border-purple-500/20 glass-panel">
        <div className="space-y-1">
          <h1 className="text-2xl font-extrabold text-white tracking-tight flex items-center space-x-2">
            <span>Agent Platform Dashboard</span>
            <span className="px-2 py-0.5 rounded-full bg-purple-500/10 border border-purple-500/30 text-purple-300 text-xs font-semibold">
              Groq LPU Active
            </span>
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="glass-card rounded-2xl p-5 space-y-2">
          ...
          <div className="flex items-baseline space-x-2">
            <span className="text-2xl font-extrabold text-white">{agents.length || metrics.summary.total_agents}</span>
        ...
        <div className="glass-card rounded-2xl p-5 space-y-2">
          ...
          <div className="flex items-baseline space-x-2">
            <span className="text-2xl font-extrabold text-white">{metrics.summary.total_conversations}</span>
            <span className="text-[10px] text-emerald-400 font-semibold flex items-center">
        ...
        <div className="glass-card rounded-2xl p-5 space-y-2">
          ...
          <div className="flex items-baseline space-x-2">
            <span className="text-2xl font-extrabold text-white">{metrics.summary.total_tool_executions}</span>
            <span className="text-[10px] text-emerald-400 font-semibold flex items-center">
        ...
        <div className="glass-card rounded-2xl p-5 space-y-2">
          ...
          <div className="flex items-baseline space-x-2">
            <span className="text-2xl font-extrabold text-white">{metrics.summary.total_tokens_used.toLocaleString()}</span>
            <span className="text-[10px] text-purple-400 font-semibold font-mono">1,200 tok/s</span>
          </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-medium text-purple-400 hover:text-purple-300 flex items-center space-x-1"
            >
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 capitalize">{agent.category} Agent</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400 pt-2 border-t border-slate-800">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-purple-600/20 border border-purple-500/30 text-purple-300 hover:bg-purple-600/30 font-medium flex items-center space-x-1 transition-all"
                  >
          ...
          <div className="glass-card rounded-2xl p-4 space-y-3 font-mono text-xs max-h-[420px] overflow-y-auto">
            {metrics.recent_executions && metrics.recent_executions.length > 0 ? (
              metrics.recent_executions.map((e: any, idx: number) => (
                <div key={idx} className="p-3 rounded-xl bg-slate-900/70 border border-slate-800/80 space-y-1.5">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="text-purple-400 font-bold">{e.model_used}</span>
                  ...
                  <div className="text-slate-300 text-[11px] flex items-center justify-between">
                    ...
                    <span className="text-emerald-400">{e.total_tokens} Tokens</span>
                  </div>
                </div>
              ))
            ) : (
              <div className="space-y-2 py-2">
                <div className="p-3 rounded-xl bg-slate-900/70 border border-slate-800/80 space-y-1">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="text-purple-400 font-semibold">llama-3.3-70b-versatile</span>
                    <span className="text-slate-500">Just now</span>
                  </div>
                  <p className="text-[11px] text-slate-300">Tool <code className="text-amber-300">calculator</code> executed in 14ms</p>
                </div>
                <div className="p-3 rounded-xl bg-slate-900/70 border border-slate-800/80 space-y-1">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="text-purple-400 font-semibold">llama-3.3-70b-versatile</span>
                  ...
                  <p className="text-[11px] text-slate-300">RAG Knowledge Chunk Retrieved (Score: 0.94)</p>
                </div>
              </div>
            )}
          </div>
