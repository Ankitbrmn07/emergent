...
export const PlaygroundPage: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-6.5rem)] flex gap-4 overflow-hidden">
      ...
      <div className="w-64 glass-panel rounded-2xl p-4 flex flex-col justify-between border border-slate-800 shrink-0">
        <div className="space-y-4">
          {/* Agent Switcher */}
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Active Agent</label>
            <select
              ...
              className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white focus:outline-none focus:border-purple-500 font-medium"
            >
              {agents.map(a => (
          ...
          <button
            ...
            className="w-full py-2 rounded-xl bg-slate-800 border border-slate-700 hover:bg-slate-700 text-white font-medium text-xs flex items-center justify-center space-x-2 transition-all"
          >
        ...
        {selectedAgent && (
          <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 text-[11px] text-slate-400 space-y-1">
            <div className="flex items-center space-x-1 text-orange-400 font-mono font-semibold">
      ...
      <div className="flex-1 glass-panel rounded-2xl border border-slate-800 flex flex-col overflow-hidden relative">
        {/* Chat Top Header */}
        <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/40">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 rounded-lg bg-purple-600/20 border border-purple-500/30 flex items-center justify-center text-purple-300 font-bold">
              <Bot className="w-4 h-4" />
            </div>
            <div>
              <h2 className="font-bold text-sm text-white">{selectedAgent?.name || 'Developer Assistant'}</h2>
              <p className="text-[10px] text-slate-400">ReAct reasoning loop with function calling enabled</p>
            </div>
          </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-5">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 max-w-md mx-auto py-12">
              <div className="w-12 h-12 rounded-2xl bg-purple-500/10 border border-purple-500/30 flex items-center justify-center text-purple-400 glow-purple">
                <Bot className="w-6 h-6" />
              </div>
              <div className="space-y-1">
                <h3 className="font-bold text-base text-white">Groq AI Agent Playground</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs w-full pt-2">
                <button
                  ...
                  className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-purple-500/40 text-left text-slate-300"
                >
                  <span className="text-purple-400 font-semibold block">Calculator Tool</span>
                  Calculate 42 * 18 + 150
                </button>
                <button
                  ...
                  className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-purple-500/40 text-left text-slate-300"
                >
                  <span className="text-purple-400 font-semibold block">Web Search Tool</span>
            ...
            messages.map((m) => (
              ...
              >
                ...
                <div className={`space-y-2 max-w-2xl ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                  ...
                  {m.tool_calls && m.tool_calls.length > 0 && (
                    <div className="space-y-1.5 mb-2">
                      {m.tool_calls.map((tc, idx) => (
                        <div key={idx} className="p-2.5 rounded-xl bg-slate-900/90 border border-purple-500/30 flex items-center justify-between text-[11px] font-mono">
                          <div className="flex items-center space-x-2">
                            <Wrench className="w-3.5 h-3.5 text-amber-400" />
                            <span className="font-bold text-slate-200">{tc.tool_name}</span>
                            <span className="text-slate-500">|</span>
                            <span className="text-emerald-400">Execution Completed</span>
                          </div>
                  ...
                  >
                    ...
                    {m.role === 'assistant' && (
                      <button
                        ...
                        className="absolute top-2 right-2 p-1 rounded bg-slate-800/80 text-slate-400 opacity-0 group-hover:opacity-100 hover:text-white transition-opacity"
                        title="Copy message"
                      >
                  ...
                  {m.latency_ms && (
                    <div className="text-[10px] text-slate-500 flex items-center space-x-2 px-1 font-mono">
          ...
          {isGenerating && (
            <div className="flex gap-3 text-xs">
              ...
              <div className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800 text-slate-400 flex items-center space-x-2 animate-pulse">
        ...
        {/* Chat Input Bar */}
        <form onSubmit={handleSendMessage} className="p-4 border-t border-slate-800 bg-slate-900/40 flex items-center space-x-3">
          <input
            type="text"
            value={inputText}
      ...
      <div className="w-72 glass-panel rounded-2xl p-4 border border-slate-800 flex flex-col justify-between shrink-0 overflow-y-auto space-y-4">
        <div className="space-y-4">
            ...
            <div className="space-y-4 text-xs font-mono">
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                  ...
                  <span className="font-bold text-white text-xs">{inspector.duration_ms} ms</span>
                </div>
                <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                  <span className="text-[10px] text-slate-500 block">Tokens</span>
                  <span className="font-bold text-purple-400 text-xs">{inspector.tokens.total}</span>
                </div>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 space-y-1 text-[11px]">
                ...
                <span className="text-amber-400 font-bold">{inspector.model}</span>
              </div>

              {/* Pending Human Approval Gate Alert */}
              {inspector.pending_approval && (
                <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/40 text-amber-300 space-y-2 font-sans">
                  <div className="flex items-center space-x-1.5 font-bold text-xs">
                    <ShieldAlert className="w-4 h-4 text-amber-400" />
                    <span>Human Approval Required</span>
                  </div>
                  <p className="text-[11px] text-slate-300">{inspector.pending_approval.action_description}</p>
                </div>
              )}

              {/* Execution Timeline Tracing */}
              <div className="space-y-2 pt-2">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block font-sans">Execution Timeline</span>
                <div className="space-y-2 border-l-2 border-slate-800 pl-3 py-1">
                  {inspector.timeline.map((ev, idx) => (
                    <div key={idx} className="space-y-0.5 text-[11px]">
                      ...
                      <p className="text-[10px] text-slate-400 font-sans line-clamp-2">{ev.details}</p>
                    </div>
                  ))}
                </div>
              </div>
