    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400">Track execution latency distribution, token usage per Groq & OpenRouter model, and tool call timelines</p>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl p-6 border border-slate-200 dark:border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-sm text-slate-900 dark:text-white flex items-center space-x-2">
            ...
            <span>Token Throughput Over Time</span>
          </h3>
          <span className="text-xs font-mono text-emerald-600 dark:text-emerald-400 font-semibold">1,200 Tokens / sec</span>
        </div>
        ...
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              <Tooltip contentStyle={{ background: '#0F172A', color: '#F8FAFC', border: '1px solid #334155', borderRadius: '12px', fontSize: '11px' }} />
            ...
            metrics.recent_executions.map((e: any, idx: number) => (
              <div key={idx} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-purple-700 dark:text-purple-300 font-bold">Execution #{e.id.substring(0, 8)}</span>
                  <span className="text-emerald-700 dark:text-emerald-400 font-bold">{e.duration_ms}ms</span>
                </div>
                <div className="text-[11px] text-slate-600 dark:text-slate-400 flex justify-between font-sans">
                  <span>Model: <code className="text-amber-700 dark:text-amber-300">{e.model_used}</code></span>
