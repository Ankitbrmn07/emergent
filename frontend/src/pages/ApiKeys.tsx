...
import { Code2, Plus, Key, Copy, Check, Trash2, ShieldCheck, Terminal } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Publish your AI agent as an external REST API endpoint (<code className="text-purple-300">POST /api/v1/agents/&#123;id&#125;/run</code>)</p>
        </div>
      </div>
      ...
      {generatedKey && (
        <div className="p-5 rounded-2xl bg-purple-600/20 border border-purple-500/50 space-y-3 glow-purple">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-amber-300 font-semibold">Won't be shown again</span>
          </div>
          <div className="flex items-center space-x-3 bg-slate-950 p-3 rounded-xl border border-slate-800 font-mono text-xs text-emerald-400">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-panel rounded-2xl p-5 border border-slate-800 space-y-4 h-fit">
          <h3 className="font-bold text-sm text-white border-b border-slate-800 pb-2">Generate New API Key</h3>
          <form onSubmit={handleCreateKey} className="space-y-3 text-xs">
            <div>
              <label className="block text-slate-300 mb-1">Key Name / Description</label>
              <input
                type="text"
                value={newKeyName}
                onChange={e => setNewKeyName(e.target.value)}
                placeholder="e.g. Production Backend Service"
                className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-white placeholder-slate-600 focus:outline-none focus:border-purple-500"
              />
            </div>
            <button
              type="submit"
        ...
        <div className="md:col-span-2 glass-panel rounded-2xl p-5 border border-slate-800 space-y-4">
          <h3 className="font-bold text-sm text-white border-b border-slate-800 pb-2">Active API Keys</h3>
          <div className="space-y-2">
            {keys.map(k => (
              <div key={k.id} className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center justify-between text-xs">
                <div>
                  <span className="font-bold text-white block">{k.name}</span>
                  <span className="text-[11px] font-mono text-purple-400">{k.key_prefix}</span>
                </div>
                <div className="flex items-center space-x-4">
                  <span className="text-[10px] font-mono text-slate-400">{k.rate_limit} req/min</span>
                  <button onClick={() => handleDeleteKey(k.id)} className="text-slate-500 hover:text-rose-400">
      ...
      <div className="glass-panel rounded-2xl p-5 border border-slate-800 space-y-3 font-mono text-xs">
        <div className="flex items-center space-x-2 text-slate-300 font-sans font-bold border-b border-slate-800 pb-2">
          <Terminal className="w-4 h-4 text-emerald-400" />
          <span>Integration cURL Snippet</span>
        </div>
        <pre className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-slate-300 overflow-x-auto text-[11px] leading-relaxed">
          {curlSnippet}
        </pre>
      </div>
    </div>
