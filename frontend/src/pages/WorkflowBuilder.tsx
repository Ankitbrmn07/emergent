...
import { GitFork, Play, Save, Plus, Layers, Bot, Wrench, Search, ShieldAlert, CheckCircle2 } from 'lucide-react';
...
export const WorkflowBuilderPage: React.FC = () => {
  ...
  const getNodeColor = (type: string) => {
    switch (type) {
      case 'start': return 'border-emerald-500/50 bg-emerald-500/10 text-emerald-400';
      case 'agent': return 'border-purple-500/50 bg-purple-500/10 text-purple-300';
      case 'knowledge_search': return 'border-indigo-500/50 bg-indigo-500/10 text-indigo-400';
      case 'condition': return 'border-amber-500/50 bg-amber-500/10 text-amber-400';
      case 'tool': return 'border-cyan-500/50 bg-cyan-500/10 text-cyan-400';
      case 'human_approval': return 'border-rose-500/50 bg-rose-500/10 text-rose-400';
      case 'end': return 'border-blue-500/50 bg-blue-500/10 text-blue-400';
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Design multi-node orchestration DAGs (Start ➔ Agent ➔ Knowledge Search ➔ Condition ➔ Tool ➔ Approval ➔ End)</p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-slate-800 border border-slate-700 hover:bg-slate-700 text-white font-medium text-xs flex items-center space-x-1.5"
          >
      ...
      <div className="glass-panel rounded-2xl p-6 border border-slate-800 space-y-6 min-h-[380px] flex flex-col justify-between relative overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
          <span className="text-xs font-semibold text-slate-300 font-mono">Node Flow Execution Pipeline</span>
          <span className="text-[10px] text-purple-400 font-mono">5 Graph Nodes Connected</span>
        </div>
          ...
          {workflowNodes.map((node, idx) => (
            <React.Fragment key={node.id}>
              <div className={`p-4 rounded-2xl border-2 space-y-1 shadow-lg text-center min-w-[140px] ${getNodeColor(node.type)}`}>
                ...
                <span className="font-bold text-xs text-white block">{node.label}</span>
              </div>
        ...
        <div className="pt-4 border-t border-slate-800/80 flex items-center space-x-3">
          <input
            ...
            className="flex-1 px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white font-mono placeholder-slate-500 focus:outline-none focus:border-purple-500"
          />
          <button
            onClick={handleRunWorkflow}
            disabled={isRunning}
      ...
      {executionResult && (
        <div className="glass-panel rounded-2xl p-5 border border-slate-800 space-y-4 font-mono text-xs">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <span className="text-purple-400 font-bold">Total Duration: {executionResult.duration_ms}ms</span>
          </div>

          <div className="space-y-2">
            <div className="text-slate-400">
              <strong className="text-slate-200">Final Output:</strong> {executionResult.final_output}
            </div>
            ...
            <div className="space-y-1.5 pt-2">
              ...
              {executionResult.execution_logs.map((log: any, idx: number) => (
                <div key={idx} className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between text-[11px]">
                  <span className="text-purple-300 font-bold">Step {log.step}: {log.label}</span>
